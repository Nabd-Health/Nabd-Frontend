...
const WeeklyScheduleCalendar = ({
  ...
  const DayCell = ({ children, value }) => {
    ...
    return (
      <div className={`h-full ${schedule?.enabled ? 'bg-[#F0FDFA]' : 'bg-[#F8FAFC]'}`}>
        {children}
      </div>
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {daysConfig.map(day => {
          ...
          return (
            <div
              key={day.key}
              className={`rounded-lg p-4 border-2 shadow-sm transition-all duration-200 ${schedule.enabled
                  ? 'bg-white border-[#1C8B8F]/20'
                  : 'bg-[#F8FAFC] opacity-75 border-[#E7ECEF]'
                }`}
              style={{ borderColor: schedule.enabled ? undefined : '#E7ECEF' }}
            >
              {/* Header with Toggle */}
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div
                    ...
                    style={{ backgroundColor: schedule.enabled ? '#1C8B8F' : '#94A3B8' }}
                  >
                    {day.name.charAt(0)}
                  </div>
                  <span className={`font-semibold ${schedule.enabled ? 'text-[#1F2E3C]' : 'text-[#64748B]'}`}>{day.name}</span>
                </div>
                ...
                <button
                  ...
                  disabled={!isEditing}
                  className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium transition-all duration-200 ${schedule.enabled
                      ? 'bg-[#F0FDFA] text-[#1C8B8F] hover:bg-[#E0F2FE]'
        ...
        <div className="md:col-span-2 lg:col-span-2 bg-[#F8FAFC] rounded-lg p-4 border border-[#E7ECEF] shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 bg-[#1C8B8F] rounded-full flex items-center justify-center">
                <FaCalendarPlus className="w-3 h-3 text-white" />
              </div>
              <h4 className="font-semibold text-[#1F2E3C] text-sm">المواعيد الاستثنائية</h4>
            </div>

            {/* Add Button - Shows form when clicked */}
            {isEditing && newException && (
              <span className="text-xs text-[#1C8B8F] font-medium">
          ...
          {isEditing && newException && onNewExceptionChange && onAddException && (
            <div className="bg-white rounded-lg p-3 border border-[#E7ECEF] mb-3">
              <div className="grid grid-cols-2 gap-2 mb-2">
                ...
                <input
                  ...
                  className="px-2 py-1.5 text-xs border border-[#E7ECEF] rounded-lg focus:ring-1 focus:ring-[#1C8B8F] focus:border-[#1C8B8F]"
                  dir="ltr"
                  placeholder="التاريخ"
                />
                ...
                <label className="flex items-center gap-1.5 px-2 py-1.5 bg-slate-50 rounded-lg cursor-pointer">
                  <input
                    ...
                    className="w-3 h-3 text-[#1C8B8F] border-gray-300 rounded focus:ring-[#1C8B8F]"
                  />
              ...
              <button
                ...
                className="w-full px-3 py-1.5 bg-[#1C8B8F] text-white text-xs font-medium rounded-lg hover:bg-[#14666A] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-1.5 transition-all duration-200"
              >
          ...
          {exceptionalDates && exceptionalDates.length > 0 ? (
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {exceptionalDates.map((exception, index) => (
                <div
                  ...
                  className="bg-white rounded-lg p-3 border border-[#E7ECEF] flex items-center justify-between hover:shadow-md transition-all duration-200"
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <FaClock className="w-3 h-3 text-[#1C8B8F]" />
                      <span className="text-xs font-semibold text-[#1F2E3C]">
            ...
            <div className="text-center py-6">
              <FaCalendarPlus className="w-8 h-8 text-[#CBD5E1] mx-auto mb-2" />
      ...
      {/* Calendar View */}
      <div className="bg-white rounded-xl p-6 shadow-lg border border-[#E7ECEF]">
        <h4 className="text-lg font-semibold text-[#1F2E3C] mb-4">عرض الجدول الأسبوعي</h4>
